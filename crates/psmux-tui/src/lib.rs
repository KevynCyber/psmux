...
//!
//! S8a is ADDITIVE AND INERT: this crate is not yet wired into any call
//! site. `ratatui` itself still stays in the root and `tests/monitor`
//! manifests until the flip slice (S8b/S8c). Widgets, `Terminal`, `Frame`,
//! and `TestBackend` are later slices -- this crate is the CORE layer only.
//!
